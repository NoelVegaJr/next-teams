...
const NewWorkspaceModal = ({ close }: { close: () => void }) => {
  const profileStore = useProfileStore();
  const profileId = profileStore.profile.id;
  const serverId = profileStore.serverId;
  ...
  const createWorkspaceHandler = async () => {
    ...
    if (newWorkspace) {
      profileStore.setActiveWorkspace(newWorkspace);
    }
    close();
  };
        ...
        <div className="p-2">
          ...
          <div className=" text-right">
            <LoadingButton
              ...
              text="create"
              className="w-full rounded bg-green-600 px-3 py-1 text-white shadow-lg transition-shadow duration-200 active:shadow-none"
            />
          </div>
        </div>
      </div>
